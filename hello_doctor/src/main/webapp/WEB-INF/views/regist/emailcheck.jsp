...
<%-- 	<form action="<%=request.getContextPath()%>/regist/phonecheck" method="post"> --%>
	<div class="row-main">
 		<div class="col-sm-12">
	           <h1 class="logo-name" style="font-size: 30px;color: black;border-bottom: solid #e7eaec;letter-spacing: -3px;">이메일 인증 확인</h1>
         </div>
 	</div>

 	<div class="row">
 		<div class="col-sm-12">
 		<input maxlength='6' oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" name="emailcheck" id="emailcheck" type="text" class="form-control" placeholder="인증번호 입력" required="">
 		</div>
 	</div>
...
$(document).ready(function(){
	...
	$.ajax({
			url:"<%=request.getContextPath()%>/regist/emailform",
			type : "POST",
...
// 		alert("인증번호가 완료되었습니다.");
		...
		window.opener.parent_emailfunction();
 		window.close();
		}, 1000);
	}else{
