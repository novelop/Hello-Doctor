				...
				<h4 class="col-md-8" style="width: 72%;">예약 내용</h4>
						...
						<form role="form" method="post" action="<%=request.getContextPath() %>/cooperation/reserveRegist.do" name="registForm">
							...
							<div class="form-group" style="display: inline-block; width: 44%;">
								...
								<div class="col-md-6" style="display: contents;">
									<div class="input-group-prepend" style="display: inline; margin-left: 2%;">
										<select id="searhlist" class="form-control" style="height: 33px; width: 69%; text-align: center; font-size: 1em; display: inline-block; margin-left: 17px;" name="searhlist">
											<option value="all" selected>선택</option>
											<c:forEach items="${selectbox }" var="i" >
										<option value="${i.coNo },${i.coDoc}">${i.pName }(${fn:substring(i.coDisease,0,8) })</option>
										</c:forEach>
							...
							<div class="form-group">
								...
								<input type="text" placeholder="하위업무 등록 +" id="worktitle" class="form-control" style="display: inline; width: 40%; margin-left: 7px; margin-left: 18px;""> <i class="fa fa-calendar" style="margin-left: 2.5%;"></i> <input
									class="datetimepicker form-control" type="text" placeholder="YYYY - mm - dd" id="workdate" style="display: inline; width: 30%; margin-left: 18px;"> <a id="append_a" onclick="apendtr()"
		 ...
		 if($('#worklist >tbody tr').length == 0 ){
		  ...
		  function makeSubname(){
			...
			$(document).on('change','#searhlist' ,function(){
				console.log()
				var coNo = $('#searhlist option:selected').val().split(',')[0];
				var coDoc = $('#searhlist option:selected').val().split(',')[1];
