...
<c:set var="myCooperationList" value="${dataMap.myCooperation}" />
...
<div class="col-sm-12" style="margin-bottom: -30px;">
	<input style="display: inline; white-space: nowrap; zoom: 1.3; vertical-align: text-top;" type="checkbox" id="chk_select" checked> <label for="chk_select" style="font-size: 1.1em;">내 전공</label>&emsp;
	...
	<div id="all-wrapper">
		<div id="inner-wrapper">
			...
			<div class="row">
				<c:forEach items="${myCooperationList}" var="myCooperation">
					<div class="col-lg-3" style="padding-left: 0px;">
						<div class="contact-box demo3" style="height: 165px; box-shadow: 1px 1px 1px 1px darkkhaki;" onclick="cooperationDetail('${myCooperation.coRelease}')">

							<div class="col-8">
								<div class="col-10" style="float:left; margin-left: -15px;">
									<h3>
										<strong>협진시작일&nbsp;&nbsp;${myCooperation.coStartDate}</strong>
										<c:if test="${coopermain.coopermain.coIngStatus eq '2'}">
										<strong>협진종료일&nbsp;&nbsp;${myCooperation.endDate}</strong>
										</c:if>
									</h3>
								</div>
								<div class="col-2" style="float:right; max-width: 21%; padding-left:35px; height: 54px;">
									<h3>
										<c:if test="${myCooperation.coRelease eq '비공개'}">
											<i style="font-size: 1.8em; color:rgb(26, 179, 148);" class="fa fa-lock"></i>
										</c:if>
										<c:if test="${myCooperation.coRelease eq '공개'}">
											<i style="font-size: 1.8em; color:rgb(26, 179, 148);" class="fa fa-unlock"></i>
										</c:if>

									</h3>
</div>
								<p>환자명 &emsp;${myCooperation.pName}</p>
								<p>주상병 &emsp;${myCooperation.coDisease}</p>
								<p>주치의 &emsp; ${myCooperation.dName} ${myCooperation.maName}
								<img alt="프로필" class="rounded-circle" src="<%=request.getContextPath()%>/doctor/getPic?dId=${myCooperation.dId}" style="display: inline-block; margin-left:10px;width: 35px; height: 35px; text-align: left; vertical-align: -21px;">
								</p>
<!-- 								<p>의료진 &emsp;</p> -->
<!-- 								<div class="doctors"> -->
<%-- 									<c:forEach items="${myCooperation.doctors}" var="doctors"> --%>
...
<!-- 								</div> -->
			...
			<div>
				<%@ include file="/WEB-INF/views/common/mypaginationCowork.jsp" %>
			</div>
		</div>
	</div>
...
$(document).ready(function(){
    $("#chk_select").change(function(){
        if($("#chk_select").is(":checked")){
            //alert("체크박스 체크했음!");
        	//subgoPage("/colist/coworklist.do");
        }else{
            //alert("체크박스 체크 해제!");
        	subgoPage("/colist/coworklist.do");
        }
