...
<input type="text" class="form-control form-control-sm m-b-xs" id="filter"
							...
							<c:forEach items="${noti }" var="i" varStatus="status">
								<tr>
									<td>${i.pName }</td>
									<td>${i.coDisease }</td>
									<td>${i.reqdId }</td>
									<td><fmt:formatDate value="${i.notiDate }" pattern="yyyy-MM-dd HH:mm"/></td>


									<td>
									<c:if test="${i.response eq '2' }">
									보류

									</c:if>

									<c:if test="${i.response eq '1' }">
									수락

									</c:if>

									<c:if test="${i.response eq '0' }">
									거절
									...
									<td><input type="button" class="btn btn-primary"onclick="javascript:location.href='historyDetail.do?comNo=${i.notiNo}'" value="상세보기"></td>
									<td>${i.coOpinion}</td>
								</tr>
							</c:forEach>
						<tfoot>
