			...
			<div style="padding: 3px">
				<div class="card card-outline card-info" style="width: 850px">
					<div class="card-header row"
						...
						<h3 class="col-md-8" style="width: 72%; text-align: center;">예약
							수정</h3>
						<div class="card-tools" style="float: right;">
							&nbsp;&nbsp;&nbsp;&nbsp;
							<button type="button" class="btn btn-outline btn-success  dim"
								onclick="regist()">수정</button>
					...
					<!--end card-header  -->
					<div id="divForm">
						<div class="card-body" id="registInfo"
							style="min-height: 582px; height: auto;">
							<h4 class="col-md-8" style="width: 72%;">예약 내용</h4>
							<form role="form" method="post"
								action="<%=request.getContextPath()%>/cooperation/reserveModify.do"
								name="registForm">
								<div style="display: none;" id="coInfoDiv"></div>
								<div class="form-group">
									<div class="col-md-6" style="display: contents;">
										<i class="fa fa-calendar" style="margin-left: 3.5%;"></i> <label
											for="date"></label> <input
											class="datetimepicker form-control" value="<fmt:formatDate value="${coTask.taDate }" pattern="yyyy-MM-dd HH:mm"/>"
											type="text" placeholder="YYYY - mm - dd"
											id="title" name="taDate"
									...
									<div class="col-md-6" style="display: contents;">
										<div class="input-group-prepend"
											...
											<input type="text" disabled="disabled" readonly="readonly"
												value="${comedvo.pName }(${fn:substring(comedvo.coDisease,0,8) })"
												id="searhlist" class="form-control"
												style="height: 33px; width: 69%; text-align: center; font-size: 1em; display: inline-block; margin-left: 17px;"
												name="searhlist"> <input type="hidden"
									...
									<div class="col-md-6" style="display: contents;">
										<div class="input-group-prepend"
											...
											<select class="form-control"
												style="height: 33px; width: 151px; text-align: center; font-size: 1em; display: inline-block; margin-left: 14px;"
												name="taName">
												<option disabled="" selected="">선택</option>
												<option  value="외래진료" ${coTask.taName eq '외래진료' ? 'selected' : ''}>외래 진료</option>
												<option value="입원치료"  ${coTask.taName eq '입원치료' ? 'selected' : ''}>입원 치료</option>
												<option value="수술"  ${coTask.taName eq '수술' ? 'selected' : ''}>수술</option>
											</select>
										</div>
									</div>
								</div>
								...
								<div class="form-group">
									...
									<div class="col-md-6" style="display: contents;">
										<textarea id="title" name="taContent" class="form-control"
											placeholder="내용을 입력하세요"
											style="width: 84%; display: inline; margin-left: 70px; height: 100px; margin-top: -26px;">${coTask.taContent }</textarea>
									</div>
								</div>
								<div class="form-group">
									...
									<input type="text" placeholder="하위업무 등록 +" id="worktitle"
										name="title" class="form-control"
										style="display: inline; width: 40%; margin-left: 7px; margin-left: 18px;"">
									<i class="fa fa-calendar" style="margin-left: 2.5%;"></i> <input
										class="datetimepicker form-control" type="text"
										placeholder="YYYY - mm - dd" id="workdate"
										name="title"
										style="display: inline; width: 30%; margin-left: 18px;">
									<a id="append_a" onclick="apendtr()"
										style="display: inline; margin-left: 11px; font-size: 23px; vertical-align: -4px;"><i
										class="fa fa-plus"></i></a>
									<table id="worklist" class="table"
										style="width: 79%; margin: 0 auto; border-bottom: 1px solid #e7eaec; text-align: center; margin-top: 20px;">
										<tbody>
										<c:forEach items="${subCoTask}" var="i">
											<tr class='append_tr'>
												<td>${i.subName}</td>
												<td><fmt:formatDate value="${i.subDate}" pattern="yyyy-MM-dd"/></td>
												<td onclick='removetr(this)'><a><i
														style='font-size: 22px;' class='fa fa-minus'></i></a></td>
											</tr>
											</c:forEach>
										</tbody>
									</table>
								...
								<input type="hidden" value=${param.taNo } name="taReference">
								</div>
							</form>
...
{{#each.}}
					<div class="vertical-timeline-block" style="">
						...
						<div class="vertical-timeline-content">
							...
							<h2
								style="margin: 5px; font-size: 15px; text-align: center;">{{dName}}</h2>
...
function regist(){
   ...
   if(taName=="선택"){
	   ...
	   return;
         ...
         swal({
                     title: "",
                     text: "예약 수정을 하기위해서는 주치의의 승인이 필요합니다. 주치의에게 승인요청하시겠습니까?",
                     type: "warning",
